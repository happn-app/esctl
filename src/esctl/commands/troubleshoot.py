...
def troubleshoot(
    ...
    health_emoji = {"green": "✅", "yellow": "⚠️", "red": "❌"}.get(
        health["status"], "❓"
    )
    print(
        f"{health_emoji} -- Cluster health: [b {health['status']}]{health['status']}[/]"
    )
    ...
    if unassigned_absent_indices:
        ...
        print(
            "    If these indices were recently deleted, the unassigned shards will eventually be cleaned up by Elasticsearch."
        )
        print(
    ...
    if system_indices:
        ...
        print(
            "    System indices are managed by Elasticsearch and should not be deleted or modified manually."
        )

        should_reroute = Confirm.ask(
            "Do you want to attempt to reroute the unassigned shards for these system indices? This may resolve the issue if the shards can be allocated to other nodes.",
            default=True,
        )
        if should_reroute:
    ...
    for shard in unassigned_shards:
        ...
        for decision in explanation.get("node_allocation_decisions", []):
            ...
            for dec in decision.get("deciders", []):
                decision_emoji = {"YES": "✅", "NO": "❌", "THROTTLE": "⚠️"}.get(
                    dec["decision"], "❓"
                )
                print(
                    f"      - {decision_emoji} {dec['decider']}: {dec['explanation']}"
                )
        ...
        if all_deciders == {"same_shard"}:
            ...
            print(
                "    This usually means that the number of nodes in the cluster is less than the number of shards for the affected indices."
            )
            print(
                "    Consider adding more nodes to the cluster or reducing the number of primary shards on the affected indices."
            )
            typer.Exit(code=1)
